...
void isPirated(){
    if([[NSFileManager defaultManager] fileExistsAtPath:@"/var/lib/dpkg/info/com.greeny.classicsiri.list"]){
        pirated = NO;
    }
}
...
@interface ClassicSiriHeaderCell : PSTableCell{
  UILabel *header;
  UILabel *footer;
...
@implementation ClassicSiriHeaderCell
    ...
    //int width = [self.specifier.target rootController].view.frame.size.width;
     ...
     if (self) {
        ...
        CGRect centeredText4 = CGRectMake(0,-500, kWidth,100);
            ...
            UIImage *image = [[UIImage alloc] initWithContentsOfFile:@"/Library/PreferenceBundles/ClassicSiri.bundle/Pingu_final.png"];
